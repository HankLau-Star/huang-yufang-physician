...
test("server-renders Huang Yufang's physician profile", async () => {
  ...
  assert.match(html, /huang-yufang-signature-portrait\.png/);
  assert.match(html, /守正为本，仁心为念/);
...
test("ships the three supplied physician photographs and accessible structure", async () => {
  ...
  assert.match(styles, /--script:\s*"Ma Shan Zheng"/);
  assert.match(styles, /font-variation-settings:\s*"wght"/);
